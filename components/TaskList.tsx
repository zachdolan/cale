    ...
    <div className="space-y-4">
      ...
      {tasks.length === 0 ? (
        <div className="py-12 text-center border-2 border-dashed border-gray-200 dark:border-gray-800 rounded-2xl">
          <p className="text-gray-400 dark:text-gray-600 font-medium">No tasks for this day yet.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {sortedTasks.map((task) => (
            <div 
              key={task.id} 
              className={`group flex items-center p-4 bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-2xl shadow-sm hover:shadow-md transition-all ${task.completed ? 'opacity-60' : ''}`}
            >
              <button 
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <h3 className={`font-semibold text-gray-800 dark:text-gray-100 ${task.completed ? 'line-through' : ''}`}>
                    {task.title}
                  </h3>
                  {task.priority === 'high' && (
                    <span className="bg-rose-100 dark:bg-rose-900/30 text-rose-600 dark:text-rose-400 text-[10px] font-bold px-1.5 py-0.5 rounded uppercase">High</span>
                  )}
                  {task.priority === 'medium' && (
                    <span className="bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 text-[10px] font-bold px-1.5 py-0.5 rounded uppercase">Med</span>
                  )}
                  {task.priority === 'low' && (
                    <span className="bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 text-[10px] font-bold px-1.5 py-0.5 rounded uppercase">Low</span>
                  )}
                  {task.endDate && task.endDate !== task.date && (
                    <span className="bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 text-[10px] font-bold px-1.5 py-0.5 rounded uppercase flex items-center gap-1">
                ...
                <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1 text-sm text-gray-500 dark:text-gray-400">
                  ...
                  {task.endDate && task.endDate !== task.date && (
                    <span className="flex items-center gap-1 shrink-0 text-indigo-500 dark:text-indigo-400 font-medium">
